...
const root = mkdtempSync(join(tmpdir(), 'tedia-edge-live-'))
  ...
  setup(buildInstance) {
    ...
    buildInstance.onLoad({ filter: /.*/, namespace: 'electron-mock' }, () => ({
      contents: `
        ...
        module.exports = {
          app: { getPath: () => process.env.TEDIAPROS_TEST_USER_DATA || os.tmpdir(), getAppPath: () => process.cwd(), isPackaged: false, getName: () => 'tedia-pros', getVersion: () => '0.1.26' },
...
try {
  await build({
    entryPoints: ['scripts/edge-tts-live-worker.ts'],
    outfile,
    bundle: true,
    platform: 'node',
    format: 'cjs',
