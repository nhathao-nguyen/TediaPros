...
- Provider `gemini-gateway` gọi CreateMediaTool tại `http://127.0.0.1:4982/openai/v1` với wire alias `gemini-advanced`, nhưng chấp nhận model văn bản thực tế Google phục vụ. Model thực tế vẫn được ghi trong audit; completion, JSON và cue ID tiếp tục được kiểm tra chặt.
- Một batch hoàn tất dùng hai gateway generation call: lượt một đọc toàn bộ ledger cue để khôi phục lỗi ASR/OCR có căn cứ và dịch; lượt hai là reviewer mới, nhận lại nguồn cùng bản nháp rồi trả JSON cuối. Nếu output bị cắt, không có bản dịch partial nào được review, checkpoint hay xuất bản: scheduler chia tập ID chưa hoàn tất thành batch nhỏ hơn, nên video dài có thể dùng nhiều hơn hai call. Nếu Gemini trả lỗi tạm thời hoặc JSON hỏng, mỗi lượt được thử tối đa ba upstream attempts; client không tạo một tầng retry generation thứ hai.
- Deadline 180 giây áp dụng riêng cho từng HTTP generation (`restore-translate` và `independent-review`), không dùng chung cho cả logical batch hai lượt. Gemini adapter sở hữu deadline từng request bằng timer/controller được giữ sống suốt request và reader cũng race từng lần đọc body với signal; orchestrator vẫn sở hữu thao tác hủy của người dùng và ngân sách toàn lượt khi chính sách ngân sách được bật lại.
- Media restoration giữ ledger OCR đầy đủ trong checkpoint nhưng chỉ gửi một observation có confidence cao nhất cho mỗi tổ hợp `text + matchingCueIds`; không gộp timestamp hay cue ID. Restoration dài được chia tuần tự tối đa 8 cue mỗi nhóm; mỗi nhóm có draft/review và operation lease riêng, sau đó toàn bộ kết quả được ghép và validate lại trên ledger gốc. Output ceiling tăng theo số cue trong khoảng 2.048–8.192 token. Cách này giảm prompt lặp theo frame và độ phức tạp mỗi generation mà vẫn giữ validator chạy trên ledger gốc.
- `outcome-unknown` nghĩa là request đã dispatch nhưng Gateway không quan sát được terminal frame; không có lịch tự mở khóa. Queue phải dừng và hiển thị thao tác khôi phục. Chỉ thao tác resume rõ ràng mới được kiểm tra receipt, xác nhận zero active/queued request, reset governor, ACK operation cũ và tạo lease mới; tự polling không được replay.
- Toàn bộ cue của một short được thử giữ trong một batch khi profile đủ capacity. Cue ID và timestamp vẫn do code quản lý; model trả compact keyed JSON `translations{cue-id:text}`, adapter dựng lại `items[{id,text}]`, rồi parser từ chối thiếu ID, thừa ID, trùng ID, Markdown hoặc sai wire route. Với cue chia nội bộ, prompt gửi mapping `unit_id` → `original_id` cùng offsets và source slice chính xác ở cả draft/review; model không phải tự đoán biên cue. Model thực tế quan sát được có thể khác route catalog miễn Gateway chứng nhận response hoàn tất.
- Prompt v10 buộc reviewer khôi phục dấu câu trên toàn bài và viết lại theo văn nói tự nhiên của locale đích. Chỉ dẫn gateway thay thế rõ quy tắc source-only cũ để prompt không tự mâu thuẫn. Budget và TTS dùng cùng partition source cố định; dấu câu đích đã review cải thiện câu chữ/caption nhưng không được tự tạo thêm ranh giới timing hoặc protected gap. Pause nguồn từ 600 ms và nhãn người nói vẫn là ranh giới cứng. Kết quả không có dấu kết câu cuối, hoặc có chuỗi dài quá 10 cue/18 giây mà không có ranh giới an toàn, bị từ chối thay vì đọc thành một đoạn sai nhịp.
...
- **Fallback Local 422 có giới hạn:** Với Local OpenAI-compatible gateway, nếu request schema-constrained nhận `HTTP 422` có body JSON khai báo không quá 4 KiB và `error.code` thuộc đúng allowlist `invalid-json`, `ambiguous-json`, `duplicate-key` hoặc `wrong-root`, client được gửi lại đúng một request cùng input nhưng không có `response_format`. Kết quả vẫn qua parser/validator như cũ; không retry mã lạ, auth/rate/server/transport error hoặc body không bị chặn kích thước. Raw error body không được đưa vào UI.
