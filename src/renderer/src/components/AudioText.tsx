...
import type { GpuInfo, WhisperModelStatus, WhisperRequest } from '../../../shared/types'
...
export default function AudioText({
  ...
  const [modelInstallPct, setModelInstallPct] = useState(0)
  const [modelInstallErr, setModelInstallErr] = useState<string | null>(null)
  ...
  useEffect(() => {
    let huy = false
    void window.api.whisperModelStatus(model).then((status) => {
      if (!huy) setModelStatus(status)
    })
    return () => {
      huy = true
    }
  ...
  const installModel = async (): Promise<void> => {
    ...
    setModelInstallErr(null)
    setModelInstallPct(0)
    const off = window.api.onWhisperModelInstallProgress((progress) => setModelInstallPct(progress.percent))
    const res = await window.api.whisperInstallModel(model)
    off()
    setModelInstalling(false)
    if (!res.ok) {
      setModelInstallErr(res.error ?? `Không thể tải model ${model}.`)
      return
    }
    setModelStatus(await window.api.whisperModelStatus(model))
  }
  ...
  const runItem = async (it: WhItem): Promise<void> => {
    const currentModelStatus = modelStatus?.id === model ? modelStatus : await window.api.whisperModelStatus(model)
    if (!currentModelStatus.complete) {
      setModelStatus(currentModelStatus)
      setModelInstallErr('Hãy cài model trước khi bắt đầu xử lý; ứng dụng không tải model trong lúc render.')
      return
    }
    setItems((prev) =>
      ...
      <div className="card options-card">
        ...
        <div className="model-readiness" style={{ marginTop: 8 }}>
          {modelStatus?.complete ? (
            ...
            <>
              <div className="bar">
                <div className="bar-fill" style={{ width: `${Math.max(0, modelInstallPct)}%` }} />
              </div>
              <div className="muted small">Đang tải và kiểm tra model… {modelInstallPct >= 0 ? `${modelInstallPct}%` : ''}</div>
            </>
          ) : (
            <div className="model-install-row">
              <div className="muted small">{modelStatus?.message ?? 'Đang kiểm tra model local…'}</div>
              {modelStatus && !modelStatus.complete && (
                <button className="btn" onClick={installModel} disabled={!hasEngine || modelInstalling}>
                  Cài model {modelStatus.id}
                </button>
              )}
            </div>
          )}
          {modelInstallErr && <div className="dy-err small">{modelInstallErr}</div>}
        </div>
