...
export function classifyTranslationError(error: unknown): TranslationFailure {
  ...
  if (providerCode === 'provider-throttled' || ['throttled', 'rate_limited', 'too_many_requests'].includes(providerCode)) return { code: 'provider-throttled', retryable: false, ...(status === undefined ? {} : { status }), ...(retryAfterMs === undefined ? {} : { retryAfterMs }), message }
