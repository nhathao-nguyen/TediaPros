...
export interface AutoShortNarratedFfmpegProcessInput {
  command: string
...
export function buildAutoShortNarratedAudioArgs(
  ...
  const duration = Math.max(0.1, input.durationSeconds).toFixed(3)
  const gain = input.bedMode === 'finite-source' ? '1.0' : originalAudioGain(input.bedVolume)
  const args = ['-y', '-hide_banner', '-nostats', '-loglevel', 'error']
  if (input.bedMode === 'looping-music') {
    args.push('-stream_loop', '-1')
  }
  args.push('-i', input.bedPath)
...
export async function runAutoShortNarratedFfmpegProcess(
  input: AutoShortNarratedFfmpegProcessInput
): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.on('error', (error) => {
      ...
      finish(new Error(`Không thể khởi động FFmpeg để trộn âm thanh nền. ${diagnostic}`))
    })
    child.on('close', (code) => {
      if (abortError) {
        finish(abortError)
...
export async function composeAutoShortNarratedAudio(input: AutoShortNarratedAudioInput): Promise<void> {
  const ffmpeg = input.ffmpegPath || (await resolveFfmpeg())
  if (!ffmpeg) throw new Error('Thiếu FFmpeg để trộn âm thanh nền với giọng lồng tiếng.')
  if (input.signal?.aborted) throw new Error('Đã hủy tác vụ')

  const args = buildAutoShortNarratedAudioArgs(input)
  await runAutoShortNarratedFfmpegProcess({
    command: ffmpeg,
    args,
    sensitivePaths: [input.bedPath, input.narrationPath, input.outputPath],
    signal: input.signal
  })
}
