...
test('Gemini Gateway keeps a 44-cue short in one batch and performs exactly draft plus review', async () => {
  ...
  try {
    ...
    assert.match(requests[0].body.messages[0].content, /source ledger/u)
    assert.match(requests[0].body.messages[0].content, /automotive wording/u)
    ...
    assert.match(requests[1].body.messages[0].content, /keyboard|exclusivity/iu)
    ...
    assert.match(audit.promptVersion, /^gemini-gateway-two-pass-/u)
