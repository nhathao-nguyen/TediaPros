...
def mask_chu(vung, cv2, np):
    """Tao binary mask cac vung co chu (trang, vang, do, xanh, stroke/vien tuong phan cao).
    Dung de phat hien doi chu giua cac khung hinh tren moi loai mau va nen."""
    ...
    # 2. Gradient canh chu (bat vien/shadow tren moi loai nen):
...
def phan_doan(files, y0, y1, x0, x1, cv2, np):
    ...
    for i, p in enumerate(files):
        ...
        if im is None:
            continue
        m = mask_chu(im[y0:y1, x0:x1], cv2, np)
        if prev is not None:
            d = jaccard(m, prev, np)
            rung[i] = d
